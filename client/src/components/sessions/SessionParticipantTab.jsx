...
const SessionParticipantTab = ({participants = [],host}) => {
    ...
    <div className='space-y-2.5'>
      {participants.map((p,idx)=>{
        ...
        const isHost = Boolean(hostId && participantUserId && participantUserId.toString()=== hostId.toString() );
         return (
          <div key={idx} className='flex items-center justify-between p-3 rounded-2xl bg-slate-500 border border-slate-100'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <span className='text-sm font-semibold text-slate-800 flex items-center gap-1.5'>{p.name}
