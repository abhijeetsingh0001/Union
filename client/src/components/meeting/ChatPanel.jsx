...
const ChatPanel = ({ isOpen, onClose, messages, onSendMessage, currentUser }) => {
  ...
  return (
    <aside className="w-full sm:w-80 h-full bg-white border-1 border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
      ...
      {/*messages container */}
      <div className='flex-1 p-4 overflow-y-auto space-y-4'>{messages.length === 0 ? (
        <div className='h-full flex flex-col items-center justify-center text-center text-slate-400 text-sm'>
          <p>
            NO messages yet
          </p>
          <p>Send a message to start chatting with Participants</p>
        </div>
      ) : (
          ...
          return (
            <div key={msg.id || index} className={`flex flex-col ${isMe ? "items-end":"items-start"}`}>
              <div className='flex items-center gap-2 mb-1'>
                ...
                <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium":" bg-slate-100 text-slate-800 rounded-tl-none border border-slate-200 font-medium"}`}>{msg.text}
