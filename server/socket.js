...
//socket.io room state:room -> map<socketid, participantObject>
const rooms = new Map();
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", async ({ roomId, user, audioEnabled = true, videoEnabled = true }) => {
      ...
      try {
        ...
        //fetch host plan to enforce participant limit (10 for free,100 for premium)
        ...
        //get existing participant in the room 
    ...
    //passes the connection details from ine user user to the other
    ...
    //host explicity ends meeting for all via end meeting button
    socket.on('end-meeting', async({roomId}) => {
      try {
        await sql`UPDATE meetings
